...
namespace System
{
    public static class StringExtensions
    {
        ...
        public static string GeneratorFileName(this string input)
        {
            string extension = Path.GetExtension(input);
            return (Path.GetFileNameWithoutExtension(input) + "_" + DateTime.Now.ToString().GetHashCode().ToString("x") + extension);
        }
        public static string UnsignNormalize(this string input)
        {
            input = input.Trim();
        ...
        public static string Unsign(this string input)
        {
            input = input.Trim();
            for (int i = 0x20; i < 0x30; i++)
            {
